...
.controller('TpechLabCtrl', ['$scope', '$http', function($scope, $http) {
	...
	$scope.sortItemNameList = function() {
		...
		var cbc = $scope.itemNameList['血'] || $scope.itemNameList.LAB;
		if (!chemo) {
			return;
		}
		...
		moveToFirst(cbc, 'Platelet');
		moveToFirst(cbc, 'Hb');
		moveToFirst(cbc, 'Neut');
		moveToFirst(cbc, 'WBC');
		moveToFirst(chemo, 'Procalcitonin');
		moveToFirst(chemo, 'Lactate');
		moveToFirst(chemo, 'CRP');
		moveToFirst(chemo, 'Albumin');
		moveToFirst(chemo, 'Phosphorus');
		moveToFirst(chemo, 'Magnesium');
		moveToFirst(chemo, 'K');
